<template>
  <div class="audio-player">
    ...
    <div class="audio-mock-player" v-if="isShowAudioPlayer">
      ...
      <!-- 上一首 -->
      <img @click="musicPlay('pre')" src="@/assets/images/vulumn.png" class="preicon" />
      <!-- "暂停" : "播放" -->
      <img @click="musicPlay('play')" class="play-sty" :src="play ? plozeIcon : playIcon" />
      <!-- 下一首 -->
      <img @click="musicPlay('next')" src="@/assets/images/vulumn.png" class="preicon" />

      <div class="right-menu">
        <div class="song-name">{{ list[index].name }}</div>
        <div class="bottom">
          <!--  播放进度条 -->
          <el-slider class="progress" v-model="sliderVal" :format-tooltip="formatTooltip" :min="sliderMin"
            :max="sliderMax" @change="spliderSelect" />

          <span class="current">{{ currentTime }}</span>
          <span class="duration">/{{ duration }}</span>
          <div class="vulumn" @click="changeVolumns">
            <!-- <img :src="require('@/assets/images/vulumn.png')" alt=""/> -->
            <!-- 有时候我们在对于一些hover定位元素，当离开时候就没了，这个时候可以使用ctrl+shift+c的快捷键来获取。 -->
            <el-popover placement="top-start" trigger="hover" popper-class="poppervulumn-class">
              <el-slider class="vulumn-progress" vertical height="100px" :step="0.1" v-model="sliderValVolumn"
                :format-tooltip="formatTooltipVolumn" :min="0" :max="1" @change="spliderSelectVolumn" />
              <img slot="reference" :src="sliderValVolumn ? vulumnIcon : mutedIcon" alt=""
...
export default {
  ...
  data () {
    return {
      ...
      vulumnIcon: require("@/assets/images/vulumn.png"),
      mutedIcon: require("@/assets/images/mute.png"),
